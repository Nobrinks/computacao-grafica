...
void display(void) {
  ...
  glColor3f(0, 1, 0);
  glPushMatrix();
    glRotatef(vectorsAngle(mouseCoord, objCoord, 2), 0, 0, 1.0);
    drawCircle(32, 0.2);
    glPushMatrix();
  ...
  glutSwapBuffers();
  glFlush();
  cout << vectorsAngle(mouseCoord, objCoord, 2) << endl;
}
...
float vectorsAngle(float* vetA, float* vetB, int vetSize) {
  float angle = acos(innerProduct(vetA, vetB, vetSize) / (norm(vetA, vetSize) * norm(vetB, vetSize))) * 180.0 / PI;
  if (vetA[0] - vetB[0] < 0) {
    return angle;
  }
  return -angle;
...
void drawCircle(int sides, float radius) {
  glBegin(GL_POLYGON);
    for(int i = 0; i < sides; i++) {
      ...
      radius * cosf(theta), radius * sinf(theta);
